...
public class Model {
	...
	public static final Set<String> IND_MNEMONICS = Set.of("CMPL", "CMPZ", "STO");
	...
	public void step() {
		...
		try {
			if(ip < currentJob.getStartcodeIndex() || ip >= currentJob.getStartcodeIndex()+currentJob.getCodeSize()) {
				...
				System.out.println(currentJob.getStartcodeIndex());
				System.out.println(currentJob.getCodeSize());
				throw new CodeAccessException("Illegal access outside of executing code");
			}
			get(getOp(ip)).execute(getArg(ip), getMode(ip));
		} catch(Exception e) {
			callBack.halt();
			throw e;
		}
