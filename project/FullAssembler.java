				...
				//check for errors associated with non blank lines in this if block
					...
					if(line.trim().toUpperCase().contains("DATA")){
						...
						if(inCode) {
							...
							errors.put(lineNum, "Duplicate DATA delimiter on line " + lineNum);
						}
						...
						if(!line.trim().contains("DATA")) {
							errors.put(lineNum, "Line does not have DATA in upper case on line " + lineNum);
						}
					}
				} 
				...
				if(line.length() > 0 && !line.toUpperCase().contains("DATA")){
					...
					//othwerise we need to make sure parts[0] and parts[1] are hexidecimal
						...
						try{
							...
							errors.put(lineNum, "Error on line" + lineNum + ": data's memory address is not a hex number");
						}
